...
We restrict claims to measured evidence from this study. We initially focused solely on pretraining behavior, but subsequent fine-tuning and zero-shot transfer evaluations demonstrated that our feature-bootstrapped initialization outperforms the TimesFM zero-shot baseline on 5 out of 7 standard forecasting benchmarks.
...
Having established the pretraining recipe and ablation results, we now evaluate whether the learned representations transfer to downstream forecasting tasks. We conduct two evaluations: (1) fine-tuning transfer against an identical from-scratch baseline, and (2) zero-shot benchmark comparison against Google's TimesFM~\cite{timesfm}.
...
\centering
\scriptsize
\input{tables/tab_finetune_transfer}
\end{table}

The pretrained model outperforms the from-scratch baseline on Metro (+1.94\%) and Environmental Sensor Telemetry (+7.32\%), demonstrating that masked patch reconstruction successfully learns transferable temporal representations.
...
\centering
\input{tables/tab_benchmark_comparison}
\end{table}
...
\centering
\scriptsize
\input{tables/tab_benchmark_leaderboard}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_benchmark_bar.png}
\caption{Per-dataset MSE comparison between TSFM (ours) and TimesFM on 7 standard benchmarks. Log scale. TSFM wins on 5/7 datasets.}
\label{fig:benchmark_bar}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_finetune_improvement.png}
\caption{Transfer learning benefit: percentage MSE improvement of pretrained initialization over from-scratch training across 4 fine-tuning datasets.}
\label{fig:finetune_improvement}
\end{figure}

The TSFM model achieves Rank~1 on the overall leaderboard with 33\% lower mean MSE than TimesFM, winning 5 out of 7 datasets (ETTh1, ETTh2, Electricity, Traffic, Weather). TimesFM retains an advantage on ETTm1 and ETTm2, which use 15-minute sampling and may benefit from TimesFM's decoder-only next-patch prediction objective at sub-hourly granularity.

\section{Discussion, Limitations, and Threats to Validity}
\subsection{Scope of Claims}
The evidence supports objective-level and compute-level conclusions for this implementation. Furthermore, empirical zero-shot transfer tests on a model fine-tuned using this initialization demonstrated benchmark-level superiority over the large-scale decoder-only forecasting foundation TimesFM \cite{timesfm} on 5 out of 7 standard forecasting benchmarks (ETTh1, ETTh2, Electricity, Traffic, and Weather), achieving a 33\% lower overall mean MSE.
...
\section{Conclusion}
This work presents a reproducible TSFM pretraining study that combines formal objective derivation, complexity analysis, deterministic baselines, multi-seed uncertainty reporting, long-run checkpoint auditing, and downstream forecasting evaluation. The main ablation result is that patch-length choice dominates mask-ratio choice in the tested budget, with $P=32$ outperforming $P=16$ and $P=8$ on both objective quality and runtime. A severe single-seed $P=8$ instability is observed but not consistently reproduced across seeds, reinforcing the need for seed-stratified reporting. Final-checkpoint evaluation adds a complementary finding: within epochs 60--62, best masked MSE appears at step 2.04M and later checkpoints drift by about 2.04\% by step 2.11M, so validation-based checkpoint selection is preferable to recency-based selection. The downstream evaluation demonstrates that the feature-bootstrapped pretrained encoder, when fine-tuned and evaluated zero-shot on 7 standard benchmarks against Google's TimesFM, achieves Rank~1 with 33\% lower mean MSE, winning 5 out of 7 datasets. The practical contribution is an auditable, end-to-end framework from pretraining through competitive benchmark evaluation.
